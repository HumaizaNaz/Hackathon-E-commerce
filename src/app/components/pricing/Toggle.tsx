...
const Toggle = ({ initialPlan }: ToggleProps) => {
  ...
  useEffect(() => {
    const fetchSubscriptions = async () => {
      try {
        const query = `*[_type == "suborder"] | order(date asc) {
          name,
          email,
          address,
          subscription-> {
        ...
        // Filter to get the most recent subscription
        ...
        const sortedSubscriptions = fetchedSubscriptions.sort((a: Subscription, b: Subscription) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const handleToggle = async (newPlan: string) => {
    setSelectedPlan(newPlan);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 px-4">
        ...
        <div className="mt-10">
          <ul className="space-y-4">
            {/* Show only the most recent subscription */}
            {subscriptions.length > 0 && (
              <li key={subscriptions[0].date} className="flex justify-between items-center w-full">
                <div className="flex items-center space-x-4">
                  {/* Plan Name */}
                  <span className={`text-red-700 font-medium`}>
                    {subscriptions[0].subscription.name}
                  </span>
                  <div className="relative">
                    <input
                      type="checkbox"
                      id="toggle"
                      className="sr-only"
                      onChange={() => handleToggle(subscriptions[0].subscription.name)}
                      checked={selectedPlan === subscriptions[0].subscription.name}
                      disabled={loading}
                    />
                    <div
                      className={`block w-20 h-10 rounded-full transition-all duration-300 ${
                        selectedPlan === subscriptions[0].subscription.name
                          ? 'bg-blue-300'
                          : 'bg-blue-200'
                      }`}
                    ></div>
